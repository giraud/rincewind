...
*)
let print_info dumpAst dumpMeta fname =
...
module Driver = struct
    ...
    let version = "0.6-snapshot"
    let usage_message = "Usage: rincewind.exe <filename>\n" ^ version
    let spec = Arg.align [ ("-d", Arg.Set dumpAst, " Dump AST") ; ("-m", Arg.Set dumpMeta, " Dump meta")]
    ...
    let () =
        try
            ...
            match !args with
                | [fname] -> print_info !dumpAst !dumpMeta fname
